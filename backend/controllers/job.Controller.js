...
export const postJob = async (req, res) => {
    try {
        ...
        return res.status(201).json({ message: "New Job Created Successfully", jobCreate, success: true });
...
export const getAdminJobs = async (req, res) => {
    try {
        ...
        if (!jobs || jobs.length === 0) {
            return res.status(404).json({ message: "No job posts found", success: false });
        }
...
export const getJobsById = async (req, res) => {
    try {
        ...
        if (!job) {
            return res.status(404).json({ message: "Job post not found", success: false });
        }
